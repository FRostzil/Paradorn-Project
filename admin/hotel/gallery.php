<?php 
 if (isset($_POST) && !empty($_POST)){ 
    ...
    if (isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])){
        ...
        $alert .= 'window.location.href = "?page='.$_GET['page'].'&function='.$_GET['function'].'&id='.$id_hotel.'";'; //แก้
        $alert .= '</script>';
        echo $alert;
        exit();
    } else {
        ...
        foreach ($query as $data):?>
            ...
            <td class="align-middle"><img src="../member/upload/hotel/<?=$data['hotel_gallery_image']?>" class="rounded" width="100" height="100">
                <td class="align-middle">
					...
					<a href="?page=<?=$_GET['page']?>&function=gallery_delete&id=<?=$data['id']?>&id_hotel=<?=$id?>" onclick="return confirm('คุณต้องการลบแกลเลอลี่ที่ : <?=$i?> หรือไม่')"; class="btn btn-sm btn-danger">ลบข้อมูล</a>                       
