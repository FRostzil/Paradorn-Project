<?php 
    $sql = "SELECT *,r.restuarant_name FROM restuarant r
            LEFT JOIN tb_travelling_type tr ON r.type_restuarant_id = tr.travelling_type_id";
?>
 <?php 
$sql = "SELECT * FROM restuarant where restuarant_username= '". $_SESSION['memberlogin'] ."' ";
        ...
        foreach ($query as $data):?>
            ...
            <td class="align-middle"><img src="../member/upload/restaurant/<?=$data['restuarant_image']?>" class="rounded" width="100" height="100">
                ...
                <td class="align-middle">
					...
					<a href="?page=<?=$_GET['page']?>&function=delete&id=<?=$data['restuarant_id']?>"onclick="showConfirmDialog(event)" <?=$data['restuarant_id']?> class="btn btn-sm btn-danger">ลบข้อมูล</a>
		
                    <script>
function showConfirmDialog(event) {
  event.preventDefault(); // ป้องกันการเปลี่ยนเส้นทาง URL ของลิงก์

  const resId = <?=$data['restuarant_id']?>;

  Swal.fire({
    title: 'คุณต้องการลบร้านอาหาร',
    text: `คุณต้องการลบร้านอาหาร : ${resId} หรือไม่`,
    icon: 'warning',
